...
def realizarBusqueda():
    ...
    for listaPermutas in listaCombinacionesPatrones:
        combinacion.clear();
        for permuta in listaPermutas:
            bit_patron = compresion(permuta, diccionario)
            combinacion.append(busqueda.divide_and_conquer_search(bitGrande,bit_patron,0));
        listasCombinacionesNaive.append(list(combinacion));
    agregarDatosNaive();
...
def agregarDatosNaive():
    global id;
    id=0;
    for listaNaive in listasCombinacionesNaive:
        for naive in listaNaive:
            cadenaNaive=",".join(map(str,naive))
            tabla.set(id, column="No. Encontrados", value=len(naive))
            tabla.set(id, column="Posiciones", value=cadenaNaive)
            id=id+1;  
    
establecerCadenaGrande();
root = tk.Tk()
img_adn = tk.PhotoImage(file="/home/gavos/Proyecto_final_Analisis_Algoritmos/adn.png").subsample(20,20);
